...
WorldModelling::WorldModelling(ros::NodeHandle &nh)
    : x_last_(0.f), y_last_(0.f), theta_last_(0.f), num_nodes_(0),
      first_node_(true), first_frontier_(true) {
  // Read parameters
  readParameters(nh);

  // Setup subscriber
  map_sub_ = nh.subscribe(input_map_topic_, 1,
                          &WorldModelling::elevationMapCallback, this);

  // Setup publisher
  graph_pub_ = nh.advertise<cdt_msgs::Graph>(output_graph_topic_, 10);
  frontiers_pub_ =
      nh.advertise<cdt_msgs::Frontiers>(output_frontiers_topic_, 10);
  traversability_pub_ =
      nh.advertise<grid_map_msgs::GridMap>(output_traversability_topic_, 10);
}
...
void WorldModelling::readParameters(ros::NodeHandle &nh) {
  ...
  nh.param("output_graph_topic", output_graph_topic_,
           std::string("/exploration_graph"));
  nh.param("output_traversability_topic", output_traversability_topic_,
           std::string("/traversability"));
  nh.param("output_frontiers_topic", output_frontiers_topic_,
  ...
  nh.param("max_distance_to_search_frontiers",
           max_distance_to_search_frontiers_, 3.f);
  nh.param("distance_to_delete_frontier", distance_to_delete_frontier_, 2.5f);
  nh.param("frontiers_search_angle_resolution",
           frontiers_search_angle_resolution_, 0.5f);
}

void WorldModelling::elevationMapCallback(
    const grid_map_msgs::GridMap &in_grid_map) {
...
bool WorldModelling::updateGraph(const float &x, const float &y,
                                 const float &theta) {
  ...
  // or the condition is satisfied, you should create a new node and add it to
  // the graph
  if (create_new_node) {
      ...
      if (dist < neighborhood_tol) {
        if (isLineTraversable(x, y, position.x, position.y)) {
          ...
          new_node.neighbors_id.push_back(
              neighbor_id); // here we fill the neighbors of the new_node
          node.neighbors_id.push_back(new_node.id);
          // ROS_ERROR("we suck");
        }
      }
    }
    // Finally add the new node to the graph (since all the properties are
    // filled)
    exploration_graph_.nodes.push_back(new_node);

    num_nodes_++; // Increase the number of added nodes

    first_node_ = false; // This is to avoid creating more than one nodes. This
                         // is just for the example, you may need to remove this
...
void WorldModelling::computeTraversability(const grid_map::GridMap &grid_map) {
  ...
  traversability_.setGeometry(grid_map.getLength(), grid_map.getResolution(),
  ...
  for (grid_map::GridMapIterator iterator(traversability_);
       !iterator.isPastEnd(); ++iterator) {
    // We only want to use the valid values
    if (traversability_.isValid(*iterator, "elevation")) {

      // TODO Fill the traversability at each position using some criterion
      ...
      if ((traversability_.at("slope", *iterator) < .3) &&
          (traversability_.at("elevation", *iterator) < 0.5)) {
...
void WorldModelling::findCurrentFrontiers(const float &x, const float &y,
                                          const float &theta,
                                          const ros::Time &time) {
  // TODO: Here you need to create "frontiers" that denote the edges of the
  // known space They're used to guide robot to new places

  // If the direction needs a frontier, create one and store in current
  // frontiers
  ...
  if (create_new_frontier) {
    ...
    first_frontier_ =
        false; // This is to avoid creating more than one frontiers. This is
               // just for the example, you may need to remove this
  }

  ROS_DEBUG_STREAM("[WorldModelling] Found "
                   << current_frontiers_.frontiers.size() << " new frontiers");
}

void WorldModelling::updateFrontiers(const float &x, const float &y,
                                     const float &theta) {
  ...
  const float step =
      traversability_
          .getResolution(); // We use the map resolution as the search step
...
void WorldModelling::publishData(
    const grid_map_msgs::GridMapInfo &in_grid_map_info) {
  ...
  traversability_msg.info.header =
      in_grid_map_info.header; // we use the same header from the original msg
                               // for consistency
...
void WorldModelling::getRobotPose(float &x, float &y, float &theta) {
  ...
  tf_listener_.waitForTransform(input_fixed_frame_, input_base_frame_,
                                ros::Time(0), ros::Duration(0.5));
  try {
    tf_listener_.lookupTransform(input_fixed_frame_, input_base_frame_,
                                 ros::Time(0), base_to_map_transform);
  ...
  // This representation is given by an axis wrt to some coordinate frame, and a
  // rotation along that axis
  Eigen::AngleAxisd axis_angle(q);

  // The value corresponding to the z component is the orientation wrt to the z
  // axis (planar rotation) We need to extract the z component of the axis and
